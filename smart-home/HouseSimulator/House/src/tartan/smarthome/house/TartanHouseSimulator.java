...
public class TartanHouseSimulator implements Runnable {
    ...
    private Boolean residentProximity;  // the state of the resident sensor (true if occupied, false if vacant)
    private Boolean intruderProximity;  // the state of the intruder sensor (true if detected)

    private Boolean lockState;          // the state of the door lock (true if locked, false if unlocked)
    private Boolean requestedLockState; // user requested state of lock
    private Boolean lockPasscodeState;  // the state of the lock passcode (true if passcode required, false if disabled)
    private String  lockPasscode;       // the lock passcode
    private String  givenLockPasscode;  // the user given lock passcode
    ...
    private final String INTRUDER_PROXIMITY = "IP";
    private final String RESIDENT_PROXIMITY = "RP";
    private final String LOCK_STATE = "LOS";
    private final String REQUESTED_LOCK_STATE = "RLS";
    private final String LOCK_PASSCODE_STATE = "LPS";
    private final String LOCK_PASSCODE = "LP";
    private final String GIVEN_LOCK_PASSCODE = "GLP";
    ...
    private final String LOCK_LOCKED = "1";
    private final String LOCK_UNLOCKED = "0";
    ...
    private final String PASSCODE_ON = "1";
    private final String PASSCODE_OFF = "0";
        ...
        public synchronized Boolean setState(Hashtable<String, Object> state) {
            ...
            for (String key : keys) {
                ...
                if (key.equals(DOOR_STATE)) {
                    ...
                    newState.append(PARAM_EQ);
                    if (newLockState) {
                        newState.append(LOCK_LOCKED);
                    } else {
                        newState.append(LOCK_UNLOCKED);
                    }
                    count++;
                    if (count < keys.size()) {
                        newState.append(PARAM_DELIM);
                    }
                } else if (key.equals(LOCK_PASSCODE_STATE)) {
                    Boolean newLockState = (Boolean) state.get(key);
                    newState.append(LOCK_PASSCODE_STATE);
                    newState.append(PARAM_EQ);
                    if (newLockState) {
                        newState.append(PASSCODE_ON);
                    } else {
                        newState.append(PASSCODE_OFF);
                    }
                    count++;
                    if (count < keys.size()) {
                        newState.append(PARAM_DELIM);
    ...
    /**
        ...
        private Hashtable<String, Object> handleStateUpdate(String stateUpdateMsg) {
            ...
            while (pt.hasMoreTokens()) {
                ...
                } else if (data[0].equals(RESIDENT_PROXIMITY)) {
                    if (val == 1) {
                        state.put(RESIDENT_PROXIMITY, true);
                    } else {
                        state.put(RESIDENT_PROXIMITY, false);
                    }
                } else if (data[0].equals(INTRUDER_PROXIMITY)) {
                    if (val == 1) {
                        state.put(INTRUDER_PROXIMITY, true);
                    } else {
                        state.put(INTRUDER_PROXIMITY, false);
                    }
