...
public class IoTConnectManager {
    ...
    public synchronized Boolean setState(Map<String, Object> state) {
        ...
        for (String key : keys) {
            ...
            if (key.equals(IoTValues.DOOR_STATE)) {
                ...
                Boolean newDoorState = (Boolean) state.get(key); // casting an expected 1 and 0 to boolean is an interesting choice but seems to work
                newState.append(IoTValues.DOOR_STATE);
                newState.append(IoTValues.PARAM_EQ);
                if (newDoorState) { //only use of converted boolean is to convert the boolean back to 1 or 0 through an if statement?
                ...
                Boolean newNightModeState = (Boolean) state.get(key);
                newState.append(IoTValues.NIGHT_MODE);
                newState.append(IoTValues.PARAM_EQ);
            ...
            } else if (key.equals(IoTValues.LOCK_STATE)) {  // following the same pattern as the previous if statement
    ...
    private Map<String,Object> handleStateUpdate(String stateUpdateMsg) {
        ...
        System.out.println(stateUpdateMsg);
        ...
        while (pt.hasMoreTokens()) {
            ...
            }  else if (data[0].equals(IoTValues.NIGHT_MODE)) {
                if (val == 1) {
                    state.put(IoTValues.NIGHT_MODE, true);
                } else {
                    state.put(IoTValues.NIGHT_MODE, false);
                }
            ...
            } else if (data[0].equals(IoTValues.RESIDENT_PROXIMITY)) {
                if (val == 1) {
                    state.put(IoTValues.RESIDENT_PROXIMITY, true);
                } else {
                    state.put(IoTValues.RESIDENT_PROXIMITY, false);
                }
            } else if (data[0].equals(IoTValues.INTRUDER_PROXIMITY)) {
                if (val == 1) {
                    state.put(IoTValues.INTRUDER_PROXIMITY, true);
                } else {
                    state.put(IoTValues.INTRUDER_PROXIMITY, false);
                }
            ...
            } else if (data[0].equals(IoTValues.NIGHT_MODE_START)) {
                state.put(IoTValues.NIGHT_MODE_START, val);
            } else if (data[0].equals(IoTValues.NIGHT_MODE_END)) {
                state.put(IoTValues.NIGHT_MODE_END, val);
            }
        }
        return state;
    }
